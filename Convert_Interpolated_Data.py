...
def main(location):
    openfile = 'data/Interpolated_data/data_' + location + '.json'
    ...
    arff.dump('data/arff_data/' + location + '_Selected_att_Clustering_Weka_Inputdata.arff', data_array,
              relation="Audi",
              names=['timestamps',
                     'accelerator_pedal', 'brake_pressure', 'steering_angle_calculated', 'vehicle_speed'])


def create_arff_file_for_all_sensors(location, df):
    ...
    data_list_allAtt = []
    for i in range(len(acc_x[:, 0])):
        data_list_allAtt.append(
            [acc_x[i][0], acc_x[i][1], acc_y[i][1], acc_z[i][1], acc_pedal[i][1], acc_pedal_grad[i][1],
    ...
    file_arff = open('data/arff_data/' + location + '_Weka_clustering_allAtt.arff', "w")
    delimiter = ","
